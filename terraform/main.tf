...
resource "aws_route53_record" "website_cert_validation" {
  count   = var.domain_name != "" ? length(aws_acm_certificate.website[0].domain_validation_options) : 0
  zone_id = aws_route53_zone.website[0].zone_id
  name    = tolist(aws_acm_certificate.website[0].domain_validation_options)[count.index].resource_record_name
  type    = tolist(aws_acm_certificate.website[0].domain_validation_options)[count.index].resource_record_type
  records = [tolist(aws_acm_certificate.website[0].domain_validation_options)[count.index].resource_record_value]
  ttl     = 60
}
